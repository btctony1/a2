...
export async function syncAndSettlePositions(
  ...
  if (isManualSync || settleItems.length > 0 || adoptedCount > 0) {
    const summaryMsg = `[对账同步完成] 交易所持仓: ${cleanOkxList.length}条, 数据库对账前: ${dbPositions.length}条, 平仓结算: ${settleItems.length}笔 (止盈:${tpCount}, 止损:${slCount}, 手动/全平:${manualCount}), 自动纳管: ${adoptedCount}笔, 当前活跃持仓: ${activePositions.length}笔, 净盈亏: ${totalSettledPnl >= 0 ? '+' : ''}${totalSettledPnl.toFixed(2)} USDT`;
