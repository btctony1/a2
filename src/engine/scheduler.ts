  ...
  for (const coin of candidateCoins) {
    try {
        ...
        if (coin.volatility_status === 'paused' || currentVol < threshold) {
          ...
          if (lastLogged !== minuteBucket) {
            ...
            await insertSystemLog(
              env,
              'info',
              `[${coin.symbol}] 智能下单过滤：上两根完整K线振幅(${currentVol.toFixed(2)}%) < 阈值(${threshold.toFixed(2)}%)，已自动暂停下单等待行情波动`
            );
          }
          continue;
        }
      ...
      if (!coin.leverage || coin.leverage <= 0 || !coin.open_interval_value || !coin.open_interval_unit) {
        await insertSystemLog(env, 'warn', `${coin.symbol} 交易参数未完整配置(杠杆/下单间隔)，跳过本轮开单`);
        continue;
      }
...
export async function syncFixedRoiFullPositionAlgoOrders(
  ...
  // 1. 快速初筛：已在内存或持久化存储中拥有有效 algoId 的币种，平台全仓策略单已生效，0 网络开销直接跳过！
  const pairsNeedingCheck: string[] = [];
  for (const pairKey of uniquePairs) {
    ...
    let cached = memoryAlgoStateCache.get(algoStateKey);
    if (!cached) {
      // 容错冷启动：若内存缓存尚未填充，检查 D1 持久化存储
      try {
        const dbVal = await getConfigValue(env, algoStateKey);
        if (dbVal) {
          const parsed = JSON.parse(dbVal);
          if (parsed && parsed.algoId) {
            cached = parsed;
            memoryAlgoStateCache.set(algoStateKey, parsed);
          }
        }
      } catch {}
    }
    if (cached && cached.algoId) {
      continue; // 核心铁律：已在平台挂载“全部仓位固定模式”委托，后续只管加仓，绝对不重复改单！0开销直接跳过！
    }
    pairsNeedingCheck.push(pairKey);
  }

  if (pairsNeedingCheck.length === 0) {
    return;
  }

  // 2. 单次批量获取全量待生效策略单与 OKX 持仓（避免逐个币种网络请求消耗配额）
  ...
  if (Array.isArray(pendingAlgos)) {
    for (const algo of pendingAlgos) {
      ...
      const clean = getCleanSymbol(sym);
      const side = algo.posSide || (algo.side === 'sell' ? 'long' : 'short');
  ...
  // 3. 逐个核对并筛选真正需要挂单/更新的币种
  const toPlaceList: Array<{
    symbol: string;
    direction: 'long' | 'short';
    coin: CoinPair;
  ...
  for (const pairKey of pairsNeedingCheck) {
    const [symbol, direction] = pairKey.split(':') as [string, 'long' | 'short'];
    const cleanSym = getCleanSymbol(symbol);
    const algoStateKey = `agg_algo_${cleanSym}_${direction}`;
    ...
    if (tpRatioPct <= 0 && slRatioPct <= 0) continue;

    // 获取持仓真实均价与实际杠杆
    const livePos = livePosMap.get(`${symbol}:${direction}`) || livePosMap.get(`${cleanSym}:${direction}`);
    let avgEntryPrice = livePos ? parseFloat(livePos.avgPx) || 0 : 0;
    let leverage = livePos ? parseFloat(livePos.lever) || coin.leverage || 10 : (coin.leverage || 10);
    ...
    if (!tpTriggerPx && !slTriggerPx) continue;

    // 检查 OKX 平台上是否已经存在该币种方向的未完成策略单
    const existingPlatformAlgo = pendingAlgoMap.get(`${symbol}:${direction}`) || pendingAlgoMap.get(`${cleanSym}:${direction}`);
    if (existingPlatformAlgo && existingPlatformAlgo.algoId) {
      const stateObj = {
        algoId: String(existingPlatformAlgo.algoId),
        tpTriggerPx: existingPlatformAlgo.tpTriggerPx || '',
        slTriggerPx: existingPlatformAlgo.slTriggerPx || '',
        updateTime: Date.now(),
      };
      memoryAlgoStateCache.set(algoStateKey, stateObj);
      setConfigValue(env, algoStateKey, JSON.stringify(stateObj)).catch(() => {});
      continue; // 核心铁律：平台已存在生效中的“全部仓位固定模式”委托，后续只管加仓，无需改单！0开销直接跳过！
    }
  ...
  if (toPlaceList.length === 0) return;

  // 4. 批量并发执行平台全部仓位策略挂单（每批 5 个，微步节奏，绝不超频控）
  for (let i = 0; i < toPlaceList.length; i += 5) {
    const chunk = toPlaceList.slice(i, i + 5);
    await Promise.all(
      chunk.map(async (item) => {
        const { symbol, direction, coin, avgEntryPrice, leverage, tpTriggerPx, slTriggerPx, algoStateKey } = item;
        const dirText = direction === 'long' ? '做多' : '做空';
        const marginMode = coin.margin_mode || 'isolated';
        const closeSide = direction === 'long' ? 'sell' : 'buy';
        // 严格对齐平台固定模式：若同时有止盈和止损使用 oco，单边使用 conditional（条件单即固定模式）
        ...
        try {
          const algoRes = await client.attachAlgoOrder({
            ...
            posSide: direction,
            ordType,
            closeFraction: '1', // 核心铁律：严格对应平台“全部仓位” 100% 规则，不传 sz
            cxlOnClosePos: true, // 核心铁律：原生布尔值与仓位深度绑定，平仓自动撤单
            reduceOnly: true,    // 核心铁律：严格只减仓
            tpTriggerPx,
            tpTriggerPxType: 'mark', // 标记价格触发，对齐平台防插针固定止盈
            tpOrdPx: '-1',           // 市价全平
            slTriggerPx,
            slTriggerPxType: 'mark', // 标记价格触发，对齐平台防插针固定止损
            slOrdPx: '-1',           // 市价全平
          });
          ...
          if (algoRes && algoRes.algoId) {
            ...
            const logTpPart = coin.tp_ratio ? `止盈 +${coin.tp_ratio}%(触发价${tpTriggerPx})` : '';
            const logSlPart = coin.sl_ratio ? `止损 -${coin.sl_ratio}%(触发价${slTriggerPx})` : '';
            const logTargetPart = [logTpPart, logSlPart].filter(Boolean).join(' / ');

            await insertSystemLog(
              env,
              'info',
              `【固定模式·全部仓位收益率】已对齐平台固定模式：${symbol}(${dirText}) 开仓均价 ${avgEntryPrice.toFixed(4)}，杠杆 ${leverage}x，目标收益率 ${logTargetPart}（全部仓位100%市价平仓，固定模式按钮已开启）`
            );
          }
        } catch (err: any) {
          console.warn(`[FixedRoiAlgo] ${symbol} 固定收益率止盈止损挂单失败:`, err?.message || String(err));
          await insertSystemLog(
            env,
            'warn',
            `【固定模式·全部仓位】${symbol}(${dirText}) 策略挂单失败: ${err?.message || String(err)}`
          );
        }
      })
    );
    if (i + 5 < toPlaceList.length) {
