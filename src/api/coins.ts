...
import { refreshCoinDirection, calculateSingleDirection, parsePeriod, type DirectionCalculationResult } from '../engine/direction';
...
import { preloadAllInstruments } from '../engine/scheduler';
...
export async function startAllCoinsHandler(
  ...
  try {
    ...
    // 2. 批量多币种K线方向研判与智能振幅计算（按周期轻量化分组批量研判）
    ...
    if (hasCredentials && coinsToCalc.length > 0) {
      ...
      // 单次拉取全量SWAP行情大包快照（1次网络请求，包含所有币种，绝不逐币拉取）
      const globalTickerMap = await liveClient.getTickersByType('SWAP').catch(() => new Map());

      for (const coin of coinsToCalc) {
        try {
          const res = await calculateSingleDirection(liveClient, coin, 'immediate', globalTickerMap);
          directionResults.set(coin.symbol, res);
        } catch (err) {
          console.warn(`[BatchEnable] ${coin.symbol} 方向研判失败:`, err);
        }
      }
    }
    ...
    // 6. 批量写入系统日志 (批量重试)
    const logList: Array<{ type: string; message: string }> = coinPairs.map((coin) => {
      const dirRes = directionResults.get(coin.symbol);
      const dirInfo = dirRes ? `，已判定方向【${dirRes.direction === 'long' ? '做多' : '做空'}】` : '';
      return {
        type: 'info',
        message: `${coin.symbol} 已独立启动自动交易 (已就绪立即开单${dirInfo})`,
      };
    });
    logList.push({
      type: 'info',
...
export async function batchUpdateCoinParamsHandler(
  ...
  try {
    ...
    if (params.sl_ratio !== undefined) cleanUpdates.sl_ratio = Number(params.sl_ratio);
...
export async function importSmartCoinsHandler(
  ...
  try {
    ...
    const slRatio = Number(rawParams.sl_ratio ?? 5);
    ...
    // 批量记录审计日志
    const logList = itemsToInsert.map((item) => ({
      type: 'info',
      message: `[优选币种导入] 导入代币 ${item.symbol}，周期: ${item.period}，初始金额: ${fundingAmount}U，等分数: ${fundingSlices}，模式: ${marginMode === 'cross' ? '全仓' : '逐仓'}，杠杆: ${leverage}X，智能波动: ${smartVolatilityEnabled ? '开(' + minVolatilityThreshold + '%)' : '关'}，加仓幅度: -${addPosRatio}%${autoStart ? ' (已自动启动交易)' : ' (手动就绪)'}`,
    }));
    await batchInsertSystemLogs(env, logList).catch(() => {});
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        sl_ratio: slRatioVal ? Number(slRatioVal) : 5,
...
export async function saveSmartConfigHandler(
  ...
  try {
    ...
    const values: Record<string, string> = {
      ...
      smart_auto_import_sl_ratio: String(Number(body.sl_ratio ?? 5)),
...
export async function executeSmartAutoImport(
  ...
  try {
    ...
    const slRatio = parseFloat((await getConfigValue(env, 'smart_auto_import_sl_ratio')) || '5');
    ...
    const logList = itemsToInsert.map((item) => ({
      type: 'info',
      message: `[定时自动优选] 自动导入全新代币 ${item.symbol}，周期: ${period}，初始资金: ${initialAmount}U，等分: ${slices}份，模式: ${marginMode === 'cross' ? '全仓' : '逐仓'}，杠杆: ${leverage}X，智能波动: ${smartVol ? '开(' + volThreshold + '%)' : '关'}，加仓幅度: -${addPosRatio}%${autoStart ? ' (自动启动交易)' : ' (待手动启动)'}`,
    }));
    await batchInsertSystemLogs(env, logList).catch(() => {});
    ...
    await insertSystemLog(
      env,
      'info',
      `[定时自动优选] 成功定时自动导入 ${itemsToInsert.length} 个全新优选代币！${autoStart ? '【已全部自动启动交易并纳入自动化调度，无需人工参与】' : '【已导入列表，等待手动启动】'}`
    );
