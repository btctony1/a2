...
.log-entry{padding:4px 0;border-bottom:1px solid #0d1117;color:#8b949e;word-break:break-all;white-space:pre-wrap;line-height:1.5}
...
.coin-card.alarm-error{border-color:#f85149!important;background:rgba(248,81,73,0.18)!important;box-shadow:0 0 0 1px rgba(248,81,73,0.3)}
  ...
  <div class="panel">
    ...
    <div class="param-config-row">
      ...
      <div class="param-grid">
        ...
        <div class="param-item">
          <label title="按全部仓位收益率(ROI%)触发止损，例如5代表全仓收益率达-5%市价全平，留空则不设置止损">止损收益率 %</label>
          <input type="text" inputmode="decimal" id="cfgSlRatio" list="dlSl" placeholder="可选 (留空不设)" oninput="markDirty(this.id)" onchange="markDirty(this.id)">
        </div>
        <div class="param-item">
  ...
  <div class="panel">
    ...
    <div id="smartCoinsPanel" class="smart-panel-box" style="display:none">
      ...
      <div class="smart-form-grid">
        ...
        <div class="smart-form-item">
          <label title="全仓止损收益率(%)，留空则不设止损">止损收益率%</label>
          <input type="number" id="smartSlRatio" step="0.5" placeholder="可选" style="width:48px">
        </div>
        <div class="smart-form-item">
...
<div id="coinParamsModal" class="modal-overlay" style="display:none">
  <div class="modal-box" style="max-width:540px;width:100%;max-height:90vh;overflow-y:auto">
    ...
    <div class="modal-grid-2">
      ...
      <div class="modal-form-group">
        <label>止损比例 (%)</label>
        <input type="number" step="any" id="mCfgSlRatio" placeholder="可选 (留空不设)">
      </div>
      <div class="modal-form-group">
        <label>等分份数</label>
...
async function loadGlobalParamsToInputs() {
  try {
    ...
    if (res && res.success && res.data) {
      ...
      setSafeInputValue('cfgSlRatio', (d.sl_ratio !== undefined && d.sl_ratio !== null && d.sl_ratio !== '') ? d.sl_ratio : '', true);
...
function startDashboard() {
  ...
  // 页面第一时间从本地缓存恢复日志，防止刷新时出现短暂白屏或被吞没
  try {
    const cachedLogs = localStorage.getItem('okx_recent_logs_cache');
    if (cachedLogs) {
      const parsed = JSON.parse(cachedLogs);
      if (Array.isArray(parsed) && parsed.length > 0) {
        currentLogsData = parsed;
        renderLogs(currentLogsData);
      }
    }
  } catch (e) {}
  ...
  try {
    ...
    if (logs && logs.success && Array.isArray(logs.data)) {
      if (logs.data.length > 0) {
        currentLogsData = logs.data;
        try {
          localStorage.setItem('okx_recent_logs_cache', JSON.stringify(logs.data));
        } catch (e) {}
      }
      renderLogs(currentLogsData);
    }
...
function renderCoins(coins) {
  ...
  displayedCoins.forEach(c => {
    ...
    const hasParams = Boolean(c.leverage && c.tp_ratio !== null && c.tp_ratio !== undefined && c.open_interval_value);
    ...
    if (hasParams) {
      ...
      const tpStr = (c.tp_ratio !== undefined && c.tp_ratio !== null && c.tp_ratio !== '') ? ('盈' + c.tp_ratio + '%') : '盈未设';
      const slStr = (c.sl_ratio !== undefined && c.sl_ratio !== null && c.sl_ratio !== '' && Number(c.sl_ratio) > 0) ? (' 损' + c.sl_ratio + '%') : '';
      ...
      paramSummary = amtStr + perSliceStr + ' | ' + (c.period || '1h') + ' | ' + lev + modeStr + ' | ' + tpStr + slStr + ' | 等分' + slices + '份/隔' + intVal + intUnit;
    ...
    cardEl.className = 'coin-card ' + (isSelected ? 'selected' : '') + ' ' + (c.error_message ? 'alarm-error' : (c.enabled ? (c.pause_open ? 'paused' : 'enabled') : ''));
    ...
    if (!cardEl.querySelector('.coin-ctrl-amt')) {
      cardEl.innerHTML = 
        ...
        '<div class="coin-ctrl-item coin-ctrl-status" title="当前代币运行与下单调度状态">' +
          ...
          (c.error_message ? ('<span class="coin-error-alarm" style="display:inline-block;font-size:10px;padding:2px 6px;border-radius:3px;background:#da3633;color:#fff;font-weight:600;margin-left:3px" title="' + c.error_message + '">⚠️ 报警: 挂单失败已停止</span>') : '') +
        '</div>' +
      ...
      let alarmTag = cardEl.querySelector('.coin-error-alarm');
      if (c.error_message) {
        if (!alarmTag) {
          const statusBox = cardEl.querySelector('.coin-ctrl-status');
          if (statusBox) {
            alarmTag = document.createElement('span');
            alarmTag.className = 'coin-error-alarm';
            alarmTag.style.cssText = 'display:inline-block;font-size:10px;padding:2px 6px;border-radius:3px;background:#da3633;color:#fff;font-weight:600;margin-left:3px';
            statusBox.appendChild(alarmTag);
          }
        }
        if (alarmTag) {
          alarmTag.textContent = '⚠️ 报警: 挂单失败已停止';
          alarmTag.title = c.error_message;
          alarmTag.style.display = 'inline-block';
        }
      } else if (alarmTag) {
...
async function saveTradeParams() {
  ...
  const payload = {
    ...
    sl_ratio: val('cfgSlRatio') !== '' ? Number(val('cfgSlRatio')) : null,
    ...
    '<div style="display:flex;flex-direction:column;gap:12px">' +
      ...
      '<div style="background:#161b22;border:1px solid #30363d;border-radius:6px;padding:10px 12px">' +
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 14px;font-size:12px">' +
          ...
          '<div style="color:#c9d1d9">止盈 / 止损: <strong style="color:#3fb950">+' + params.tp_ratio + '%</strong> / ' + (params.sl_ratio !== null ? ('<strong style="color:#f85149">-' + params.sl_ratio + '%</strong>') : '<span style="color:#8b949e">无止损</span>') + '</div>' +
...
async function loadSmartConfig() {
  try {
    ...
    if (res && res.success && res.data) {
      ...
      if (document.getElementById('smartTpRatio')) document.getElementById('smartTpRatio').value = (cfg.tp_ratio !== undefined && cfg.tp_ratio !== null && cfg.tp_ratio !== '') ? cfg.tp_ratio : 5;
      if (document.getElementById('smartSlRatio')) document.getElementById('smartSlRatio').value = (cfg.sl_ratio !== undefined && cfg.sl_ratio !== null && cfg.sl_ratio !== '') ? cfg.sl_ratio : '';
...
async function saveSmartConfig(btn) {
  ...
  const slInputVal = document.getElementById('smartSlRatio')?.value?.trim();
  const slRatio = (slInputVal !== undefined && slInputVal !== '' && !isNaN(parseFloat(slInputVal))) ? parseFloat(slInputVal) : null;
...
async function importSelectedSmartCoins(btn) {
  ...
  const intervalUnit = document.getElementById('smartIntervalUnit')?.value || 'hour';
  const tpRatio = parseFloat(document.getElementById('smartTpRatio')?.value) || 5;
  const slValImport = document.getElementById('smartSlRatio')?.value?.trim();
  const slRatio = (slValImport !== undefined && slValImport !== '' && !isNaN(parseFloat(slValImport))) ? parseFloat(slValImport) : null;
  const smartVol = document.getElementById('smartSmartVolatility')?.value === '1';
  const volThreshold = parseFloat(document.getElementById('smartMinVolatility')?.value) || 1.0;
  const addPosRatio = parseFloat(document.getElementById('smartAddPosRatio')?.value) || 0;
  const profitTransfer = parseFloat(document.getElementById('smartProfitTransferRatio')?.value) || 0;
...
async function toggleCoin(symbol, enabled) {
  if (enabled) {
    ...
    if (coin) {
      ...
      if (!coin.leverage || coin.tp_ratio === null || coin.tp_ratio === undefined || !coin.open_interval_value) {
        alert('【' + symbol.replace('-USDT-SWAP','') + '】尚未配置完整的交易参数(杠杆/止盈/下单间隔)，请先在上方配置并保存交易参数后再启动交易！');
        return;
      }
    }
  }
...
async function clearLogs() {
  ...
  if (res.success) {
    currentLogsData = [];
    try { localStorage.removeItem('okx_recent_logs_cache'); } catch (e) {}
    renderLogs(currentLogsData);
    await loadLogs();
  } else {
...
function openCoinSettingsModal(symbol, event) {
  ...
  setVal('mCfgSlRatio', (coin.sl_ratio !== undefined && coin.sl_ratio !== null && coin.sl_ratio !== '') ? coin.sl_ratio : '');
...
async function submitModalCoinParams() {
  ...
  const payload = {
    ...
    sl_ratio: val('mCfgSlRatio') !== '' ? Number(val('mCfgSlRatio')) : null,
